...
sys.path.append("/dev/src/second.pytorch/second")

from pytorch.train import evaluate
...
def velo_callback(msg):
    ...
    start = time.clock()
    results = evaluate(np_p_ranged, config_path="/dev/src/second.pytorch/second/configs/car.fhd.config",
                       model_dir="/root/model/new_model",
...
if __name__ == '__main__':
    ...
    sub_ = rospy.Subscriber("/pandar_points", PointCloud2, velo_callback, queue_size=1)
    ...
    pub_arr_bbox = rospy.Publisher("voxelnet_arr_bbox", BoundingBoxArray, queue_size=1)
    print("arr bbox over")
