                ...
                if global_step % display_step == 0:
                    ...
                    for k, v in flatted_summarys.items():
                         ...
                         if isinstance(v, (list, tuple)):
                            ...
                            # print("v", v, type(v))
                            # print("global_step", global_step, type(global_step))
...
def _predict_kitti_to_file(net,
    ...
    # print("predictions_dicts", predictions_dicts)
    # t = time.time()
...
def evaluate(config_path,
    ...
    return net,eval_dataset,eval_dataloader,train_cfg,model_cfg,target_assigner,model_dir
def test(net,
         eval_dataset,
         eval_dataloader,
         train_cfg,
    ...
    if measure_time:
        ...
        all_time = np.mean(prep_example_times) * 1000 + np.mean(prep_times) * 1000
    for name, val in net.get_avg_time_dict().items():
        print(f"avg {name} time = {val * 1000:.3f} ms")
        all_time = all_time +val * 1000
    print(f"all_time ={all_time:.3f} ms")
    if not predict_test:
...
if __name__ == '__main__':
    #fire.Fire()
    net,eval_dataset,eval_dataloader,train_cfg,model_cfg,target_assigner,model_dir=evaluate(config_path="/root/second_version1.5/second/configs/car.fhd.config", model_dir="/root/model/222/", batch_size=1, pickle_result=False)
    test(net=net,
