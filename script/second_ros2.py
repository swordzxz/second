...
def velo_callback(msg):
    ...
    print("read_points time:",time.time()-read_points)
    # print("pcl_msg",type(pcl_msg))
    # print("pcl_msg", type(list(pcl_msg)))
    # print("generator")
    # for p in pcl_msg:
    #     print("p", p)
    ...
    cond = hv_in_range(x=np_p[:, 0],
                       ...
                       fov=[-35,35],
                       fov_type='h')
    print("cond_time",time.time()-cond_time)
    #视野范围-180,180
    # cond = hv_in_range(x=np_p[:, 0],
    #                    y=np_p[:, 1],
    #                    z=np_p[:, 2],
    #                    fov=[-180, 180],
    #                    fov_type='h')
    cut_points =time.time()
    np_p_ranged = np_p[cond]
    print("cut_points",time.time()-cut_points)
    ...
    print("create_box time:",time.time()-create_box)
    ...
    if len(arr_bbox.boxes) is not 0:
        ...
        print("np_p_ranged",np_p_ranged.shape)
        publish_test(np_p_ranged, msg.header.frame_id)
        print("publish_points_time", time.time() - publish_points_time)


        # publish to /voxelnet_arr_bbox
        pubulish_bbox_time=time.time()
        pub_arr_bbox.publish(arr_bbox)
        print("publish_bbox_time :",time.time()-pubulish_bbox_time)
